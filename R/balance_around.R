## BalancePTS.R - 6/11/2018; SLGarman - BLM NOC, Denver.

## There are 2 options.

#  OPTION I.
## A way to add and balance NEW GRTS points around an existing GRTS design.  Determines and eliminates the closest New pts from existing GRTS pts.  Given N
## existing points, the New points should consist of N plus the desired number of New pts.  E.g., if 52 existing pts and you want to add another 52, use GRTS to derive 104 pts, then
## run the existing and new pts shapefiles thru this script.  This script will identify the 52 New points to eliminate, and produce an integrated, balanced design with 104 points.
## To check the results, run the output points shapefile through randomizepv4.r.  The output shapefile contains both existing and selected New points.  Existing points
## have CODE set to 1, and New points have CODE set to 2 so you can extract just the New points, where necessary.

## There are only 2 inputs - a shapefile with existing GRTS points and a shapefile with the Newly selected points - the number and names of shapefile attributes must be identical!
## The existing points are ingested first and $CODE<-1 designates these points, then the New-design points are ingested and $CODE<-2 designates these points.
## For each of the N existing points, the closest New point is determined.  Where multiple existing points have the same
## closest New point, the existing point with the longer distance is determined and it's next closest New point is selected.  Checking and
## resolving which New points are closest to existing points such that New points are not selected more than once is iterative, and terminates when N number of unique New points
## are selected, where N is the number of existing GRTS points.  These N new points are then eliminated, producing a new integrated design.
## This only works if the SAME frame/strata is used for both existing and New points.

## The above procedures operate at the extent of an entire sample frame OR on a strata basis.  For the latter, you must specify the name of the strata field in
## your input point shapefiles - spatial balance is then evaluated by individual stratum.   If this field name is not provided (set to NA),
## the ingested shapefiles are assumed to represent an entire frame, and spatial balance is based on the entire collection of existing and New points.  If
## both the existing and New designs are limited to just 1 stratum, however, then you effectively have just 1 'frame' and all points pertain to that single frame area.  In this
## situation you can run this script without designating a stratum field.

## When adding New strata points, only those strata with more New points than existing points are balanced.  Say the existing design has points in strata A and B.  You can
## add more A points and also add points to other strata, say stratum=C.  The A points will be balanced. The output shapefile will have these balanced A points along
## with the original B points and the new C points.  In short, we balance the strata where New points outnumber the existing points, and carry-over everything else
## from the existing and the New point files into the final output.  If the number of New points is <= the number of existing points in a strata, there is no attempt
## to balance designs, and you get all of the existing and new points in the final output (aka, you get a collection of junk cause you tried to balance designs and didn't
## supply a sufficient number of New points).  The number of New points must exceed the number of existing points (for an entire frame or by strata) for the balance algorithm
## to properly function.


## OPTION II.
## As an add-on, this script can take a set of N existing pts and select pts that are closest to those of a GRTS new-draw having <N pts (where N is by strata).  This is
## useful when you are trying to ID pts for revisitation from existing pts that are highly unbalanced.  This option usurps the pt selection component of selectpts.R which
## otherwise is used to ID revisitation pts from an existing 5-yr design.  This option was an initial attempt to provide a combination of existing (but not all existing) and
## new points in a balanced manner.  A more developed method was created to perform this operation and is called revisist.r.  It is advised not to use option 2
## since it is not well developed or tested.

## NOTE:  Always have problems if shapefiles have a z dimension.  Make sure all inputs have only 2 dimensions.

## How to run the script.
# I.  Copy all of section I into the R console
# II. Modify I/O arguments in section II then cut and paste into R console
# OR batch it:   R CMD BATCH balancepts.r capture_console_output
