...
search <- function(dataframe = NULL,
  ...
  if(!is.null(dataframe)) {
    if (grepl(class(dataframe), pattern = "^(Spatial).{1,100}(DataFrame)$")) {
      dataframe <- dataframe@data
    }
  }# else {
  #  stop("df must be a data frame or a spatial object with a @data slot")
  #}

  values <- unique(values)

  ## Get the fieldname as it occurs in the data frame
  fieldname <- find_name(obj = dataframe,
                         name = namestring,
  ...
  if (use_grepl | ignore_case_values) {
    ...
    output <- dataframe[grepl(dataframe[, fieldname],
    ...
    output <- dataframe[dataframe[[fieldname]] %in% values, ]
  }
