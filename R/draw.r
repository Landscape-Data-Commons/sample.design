...
#' @param panel.names A character vector of the names to assign to the panels for automatically generating a proprtional-allocation design object. All values must be unique. This must be the same length and in the same order as \code{panel.sample.size}. Defaults to \code{c("One", "Two", "Three", "Four", "Five")}.
#' @param panel.sample.size A numeric vector of the number of base points to be drawn in each panel for automatically generating a proprtional-allocation design object. This must be the same length and in the same order as \code{panel.names} UNLESS all panels have the same number of base points in which case \code{panel.sample.size} optionally may be a single numeric value despite \code{panel.names} containing more than one value. Defaults to \code{c(50)}.
#' @param points.min A numeric value of the minimum number of base points to allocate to a stratum within a panel regardless of its relative size for automatically generating a proprtional-allocation design object. Defaults to \code{3}.
...
#' @param oversample.min A numeric value representing the minimum number of oversample points to allocate per stratum per panel for automatically generating a proprtional-allocation design object. This is only used if it is greater than \code{panel.sample.size * min.oversample.proportion}. Defaults to \code{3}.
